...
import com.robolearn.course.entity.Module.CurriculumItem;
...
public class JavaCourseSeeder {
    ...
    public void seedJavaCourse() {
        List<Course> existingCourses = courseRepository.findAllByTitle("Java Mastery: Zero to Hero");
        if (!existingCourses.isEmpty()) {
            courseRepository.deleteAll(existingCourses);
            log.info("Deleted {} existing Java course(s) to force re-seed.", existingCourses.size());
        }
